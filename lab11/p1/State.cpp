...
int State::approx_distance(const State other) {
 	int approx_distance = -1;
	/*TODO: functie admisibila care sa estimeze costul pana la starea finala*/ 

//	approx_distance = ABS(other.x - x) + ABS(other.y - y);
	
//	approx_distance = sqrt (ABS(other.x - x) * ABS(other.x - x) + ABS(other.y - y) * ABS(other.y - y));
	
	approx_distance = other.x + x + other.y + y;
