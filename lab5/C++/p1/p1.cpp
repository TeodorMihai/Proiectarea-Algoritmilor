...
int Nim::eval(int player) {
    ...
    //daca fac miscarea, adversarul meu e intr-o stare P
    if(notdivby4 % 2 == 0) //stare finala  
            res = 1;
         else 
            res =  -1;
...
pair<int, Move> maxi(Nim state, int player, int depth) {
    ...
    for(Move m : moves) {
        ...
        pair<int, Move> res_mini = maxi(next_state, player, depth - 1);
...
pair<int, Move> mini(Nim state, int player, int depth) {
    ...
    if(depth == 0 || state.ended())
        return  pair<int, Move> (state.eval(player) , Move(0, 0) ) ;
    //vreaus a fac cea mai buna miscare dar state.eval returneaza
    //1 daca eu pot castiga, -1 daca nu, si avand in vedere ca aleg minimul, trebuie
    //sa aleg -eval
    int minv = Inf ;
    Move best_move;
