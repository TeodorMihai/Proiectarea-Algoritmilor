...
pair<int, Move> maxi(Reversi , int, int, int, int);

pair<int, Move> mini(Reversi state, int player, int depth, int alfa, int beta) {
    ...
    //beta e minimul dintre valorile fiilor
    cnt++;
    ...
    for(Move m : moves) {
        ...
        //in loc de alfa pot pune -Inf (se alege maximul), de ce merge si cu afla?
        pair<int, Move> res_maxi = maxi(next_state, -1 * player, depth - 1, -Inf,  minv);
        
        int score = res_maxi.first;
        //alfa e maximul pe care l-am gasit pana acum pe nivel
        //daca gasesc un score < alfa, insemna ca rezultatulul miniului astuia va fi 
        // mai mic decat sau egal ca score (se alege minimul), deci nu va fi propagat mai departe
        //pentru ca va fi propoagat alfa care e mai mare


        if(score <= alfa) 
            return make_pair(score, m);//returneaza score pentru ca el este scoreul
                                       //care va fi ales si implicit cel mai mic
...
pair<int, Move> maxi(Reversi state, int player, int depth, int alfa, int beta) {
    ...
    for(Move m : moves) {
        ...
        next_state.apply_move(m);
        //pot sa propag in loc de beta Inf, de ce merge si cu beta, e tot aia oricum, de ce?
        pair<int, Move> res_mini = mini(next_state, -1 *  player, depth - 1, maxv, Inf);
        
        int score = res_mini.first;

       if(score >= beta) 
            return make_pair(score , m);//the move returned matters because  
